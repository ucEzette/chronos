  ...
  return (
    <div className="fixed inset-0 bg-[#050b14] flex flex-col items-center justify-center relative overflow-hidden z-[100] font-display text-primary select-none px-4">
      ...
      <div className="relative z-10 flex flex-col items-center w-full max-w-sm">
        <div className="w-32 h-32 md:w-40 md:h-40 rounded-full border-2 border-primary shadow-[0_0_30px_rgba(0,229,255,0.3),inset_0_0_30px_rgba(0,229,255,0.2)] relative flex items-center justify-center mb-8 md:mb-12 group">
          ...
          <div className="w-16 h-16 md:w-20 md:h-20 bg-primary shadow-[0_0_40px_rgba(0,229,255,0.8)] clip-path-polygon-[50%_0%,100%_25%,100%_75%,50%_100%,0%_75%,0%_25%] animate-pulse"></div>
        </div>

        <h1 className="text-4xl md:text-5xl font-black tracking-[0.2em] mb-4 text-transparent bg-clip-text bg-gradient-to-b from-white to-primary drop-shadow-[0_0_10px_rgba(0,229,255,0.5)] text-center">
          CHRONOS
        </h1>
        <p className="text-xs md:text-sm tracking-[0.3em] uppercase mb-12 text-primary/70 text-center">
        ...
        {!loaded ? (
          <div className="w-full mb-8">
             ...
             <div className="h-1.5 bg-primary/20 rounded-full overflow-hidden relative">
                 <div className="h-full bg-primary shadow-[0_0_20px_rgba(0,229,255,0.8)] transition-all duration-300 ease-out relative" style={{ width: `${Math.min(progress, 100)}%` }}></div>
             </div>
          </div>
        ) : (
          <button 
            onClick={onEnter}
            className="relative group w-full px-10 py-3 bg-transparent border-2 border-primary text-primary font-bold uppercase tracking-wider overflow-hidden transition-all duration-300 hover:bg-primary hover:text-[#050b14] active:scale-95 animate-in fade-in zoom-in-95 rounded-sm"
          >
...
function MarketplaceCard({ item }: { item: any }) {
  ...
  return (
    <div className="group relative bg-surface/60 backdrop-blur-md border border-white/10 rounded-2xl overflow-hidden hover:border-primary/50 transition-all duration-300 hover:shadow-neon flex flex-col">
      <div className="absolute top-3 right-3 z-20"><span className="inline-flex items-center rounded-full bg-black/80 backdrop-blur-md px-2.5 py-1 text-[10px] font-bold text-primary border border-primary/30 shadow-sm">{type}</span></div>
      ...
      <div className="p-4 flex flex-col gap-3 flex-grow">
        ...
        {showDetails && <div className="bg-white/5 p-3 rounded-lg text-xs text-white/70 animate-in slide-in-from-top-2 font-mono border border-white/10"><h4 className="flex items-center gap-1 font-bold text-white mb-1 uppercase"><Info size={12}/> Description</h4><p className="mb-2 leading-relaxed opacity-80">{meta?.description || "Encrypted artifact details are secured on-chain. Purchase to access full content."}</p><div className="flex items-center gap-1 pt-2 border-t border-white/10 text-white/40"><Box size={10}/> ID: <span className="truncate">{item.id.toString()}</span></div></div>}
        ...
        <div className="mt-auto pt-3 border-t border-white/5 flex items-center justify-between gap-3">
          ...
          <div className="flex gap-2">
            ...
            <button disabled={isSoldOut} className={cn("px-4 py-2 rounded-lg text-xs font-bold transition-all shadow-lg flex items-center gap-2 uppercase tracking-wide flex-1 justify-center active:scale-95", isSoldOut ? "bg-white/5 text-white/20 cursor-not-allowed border border-white/5" : "bg-primary hover:bg-white text-black shadow-neon hover:shadow-white/20")}>{isSoldOut ? "Unavailable" : "Buy Now"}</button>
          </div>
        </div>
      </div>
    </div>
      ...
      <main className="max-w-[1440px] mx-auto px-4 md:px-6 py-8 md:py-12 relative z-10">
        ...
        <div className="flex flex-wrap gap-2 mb-8 overflow-x-auto pb-2 scrollbar-hide">
          {["ALL", "AUDIO", "VIDEO", "DATA", "ARCHIVE"].map((f) => (
            <button key={f} onClick={() => setFilter(f)} className={cn("px-4 py-2 rounded-lg text-xs font-bold font-mono transition-all border whitespace-nowrap", filter === f ? "bg-primary/20 text-primary border-primary shadow-neon" : "bg-surface text-white/60 border-white/10 hover:border-white/30 hover:text-white")}>{f}</button>
          ))}
        </div>
