...
// Helper
const formatTimeAgo = (t: number) => new Date(t * 1000).toLocaleDateString();
function Toast({ message, type, onClose }: { message: string, type: 'success' | 'error', onClose: () => void }) {
  useEffect(() => { const t = setTimeout(onClose, 5000); return () => clearTimeout(t); }, [onClose]);
  return (<div className={cn("fixed bottom-10 right-6 z-[100] flex items-center gap-3 px-6 py-4 rounded-xl border backdrop-blur-xl animate-in slide-in-from-right shadow-2xl", type === 'success' ? "bg-primary/10 border-primary/30 text-primary shadow-glow-primary" : "bg-red-500/10 border-red-500/30 text-red-400")}>{type === 'success' ? <CheckCircle2 size={20} /> : <AlertCircle size={20} />}<p className="text-sm font-bold font-mono uppercase tracking-wider">{message}</p><button onClick={onClose}><X size={16} /></button></div>);
}
...
export default function DashboardPage() {
  ...
  const { data: rawItems, refetch: refetchItems } = useReadContract({ address: PAYLOCK_ADDRESS, abi: PAYLOCK_ABI, functionName: 'getMarketplaceItems' });
  const allItems = (rawItems as any[]) || [];

  const { data: ownershipData } = useReadContracts({
    contracts: allItems.map((item) => ({ address: PAYLOCK_ADDRESS, abi: PAYLOCK_ABI, functionName: 'checkOwnership', args: [item.id, address] })),
  ...
  const unifiedFeed = useMemo(() => {
    ...
    allItems.forEach((item: any, index: number) => {
      ...
      if (ownership && ownership[0] === true) feed.push({ ...item, type: 'ACQUISITION', buyer: address });
    });
    return feed.reverse();
    ...
    <div className="min-h-screen bg-background text-white font-display overflow-x-hidden">
      ...
      <main className="max-w-[1280px] mx-auto px-4 md:px-6 py-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          ...
          <button className="flex items-center gap-2 px-5 py-2.5 bg-primary hover:bg-primary-dark text-black text-sm font-bold rounded-lg shadow-neon hover:scale-105 transition-all w-full md:w-auto justify-center"><Plus size={18}/> Upload New File</button>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-6 mb-12">
          {/* Revenue */}
          <div className="bg-surface/60 border border-white/10 rounded-xl p-6 backdrop-blur-md hover:border-primary/50 transition-all group">
          ...
          {/* Items Bought */}
          <div className="bg-surface/60 border border-white/10 rounded-xl p-6 backdrop-blur-md hover:border-green-500/50 transition-all group">
          ...
          {/* Files Sold */}
          <div className="bg-surface/60 border border-white/10 rounded-xl p-6 backdrop-blur-md hover:border-secondary/50 transition-all group">
          ...
          {/* Pending Keys */}
          <div className="bg-surface/60 border border-white/10 rounded-xl p-6 backdrop-blur-md hover:border-warning/50 transition-all group">
        ...
        <div className="w-full overflow-hidden rounded-xl border border-white/10 bg-surface/60 backdrop-blur-md shadow-2xl">
                  ...
                  return (
                    <tr key={i} className="group hover:bg-white/5 transition-colors">
                      ...
                      <td className="py-4 px-6 text-center">
                        <span className={cn("inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold border shadow-sm", item.isDelivered ? "bg-success/10 text-success border-success/20" : item.isCancelled ? "bg-danger/10 text-danger border-danger/20" : item.type === 'SALE' ? "bg-warning/10 text-warning border-warning/20" : "bg-primary/10 text-primary border-primary/20")}>{item.isCancelled ? "CANCELED" : item.isDelivered ? "DELIVERED" : item.type === 'SALE' ? "WAITING" : "LISTED"}</span>
                      </td>
                      <td className="py-4 px-6 text-right">
                        {!item.isDelivered && !item.isCancelled && item.type === 'SALE' && (
                          <button onClick={() => handleDeliver(item)} disabled={!!processingId} className="bg-primary hover:bg-white text-black px-4 py-1.5 rounded text-xs font-bold shadow-neon transition-all flex items-center justify-center gap-2 ml-auto">{processingId === item.id.toString() ? <Loader2 className="animate-spin" size={14}/> : <Key size={14}/>} DELIVER</button>
                        )}
                      </td>
                    </tr>
                  );
