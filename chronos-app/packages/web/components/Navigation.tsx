...
import { Terminal, Power, Wallet, LogOut } from "lucide-react";

export function Navigation() {
  const { address, isConnected } = useAccount();
  const { disconnect } = useDisconnect();
  const { data: balance } = useBalance({ address }); // Fetch Balance
      ...
      <header className={cn(
        ...
        <div className="max-w-[1440px] mx-auto px-6 lg:px-12 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 cursor-pointer group" onClick={() => router.push("/")}>
            <div className="size-8 rounded bg-primary/20 flex items-center justify-center text-primary border border-primary/30 shadow-[0_0_15px_rgba(6,182,212,0.3)]">
              <Terminal size={18} />
            </div>
          ...
          <nav className="hidden md:flex gap-8 text-xs font-bold uppercase tracking-widest">
            ...
            <button onClick={() => router.push("/dashboard")} className={cn("hover:text-primary transition-colors", pathname === "/dashboard" && "text-primary")}>Dashboard</button>
            <button onClick={() => router.push("/profile")} className={cn("hover:text-primary transition-colors", pathname === "/profile" && "text-primary")}>Profile</button>
          </nav>
          ...
          <div>
            {isConnected ? (
              <div className="flex items-center gap-4">
                {/* Balance & Address */}
                <div className="text-right hidden sm:block bg-white/5 px-3 py-1.5 rounded-lg border border-white/10">
