...
export const getAudioSnippet = async (audioFile: File): Promise<Blob | null> => {
  try {
    ...
    const offlineContext = new OfflineAudioContext(audioBuffer.numberOfChannels, duration * audioBuffer.sampleRate, audioBuffer.sampleRate);
